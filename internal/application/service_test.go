...
func TestLTP_DecimalPrecision(t *testing.T) {
	testCases := []struct {
		...
		{"Precisión alta", "123.4567890123456789", "123.4567890123456789"},
		{"Valor exacto 0.1", "0.1", "0.1"},
		{"Valor exacto 0.2", "0.2", "0.2"},
		{"Suma problemática", "0.1", "0.1"},
		{"Valor grande", "1000000.00000001", "1000000.00000001"},
		{"Valor pequeño", "0.0000000000000001", "0.0000000000000001"},
	}
...
func TestLTP_JSONPrecision(t *testing.T) {
	testCases := []struct {
		...
		{"Precisión alta", "123.4567890123456789"},
		{"Valor exacto 0.1", "0.1"},
		{"Valor exacto 0.2", "0.2"},
	}
...
func TestLTP_ArithmeticPrecision(t *testing.T) {
	...
	assert.True(t, result.Equal(expected),
		"0.1 + 0.2 debería ser exactamente 0.3, got %s", result.String())
}
...
func TestLTP_VerifyConsistency(t *testing.T) {
	testCases := []struct {
		...
		{"Precisión normal", "50000.00"},
		{"Precisión alta", "50000.1234567890123456"},
		{"Valor problemático", "0.1"},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.NoError(t, err, "La consistencia del LTP no se mantuvo")
		})
	}
}
...
func TestLTP_PrecisionInServiceOperations(t *testing.T) {
	...
	assert.True(t, precisionLTP.Amount.Equal(result.Amount),
		"Precisión perdida en operación de servicio: esperado %s, obtenido %s",
...
func TestLTP_EdgeCasePrecision(t *testing.T) {
	edgeCases := []struct {
		...
		{"Máxima precisión", "1.12345678901234567890", "1.1234567890123456789"},
		{"Valor cero", "0.0", "0"},
		{"Valor negativo", "-1.5", "-1.5"},
		{"Notación científica", "1e-10", "0.0000000001"},
	}
	...
	for _, tc := range edgeCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err != nil {
				t.Logf("Caso %s generó error (puede ser esperado): %v", tc.name, err)
				return
			}
